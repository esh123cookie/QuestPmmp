		...
		public function giveAward($player, $award, $id) {
			if($award['id'] == 'money') {
				if(!isset($award['count'])) {
					$this->getLogger()->warning("§lYou have not specified the Amount! (count)");
					return true;
				}
				$this->plugin->eco->giveMoney($player->getName(), $award['count']);
				$player->sendMessage(str_replace(['{quest}', '{award}', '{count}'], [$this->plugin->config['quests'][$id]['name'], isset($award['name']) ? $award['name'] : 'Coins', $award['count']], $this->plugin->config['questEnd']));
				$player->addTitle("§aComplete!","§e> New quest Now Open! <");
				return true;
			}
			if($award['id'] == 'other') {
				Server::getInstance()->dispatchCommand(new ConsoleCommandSender(), str_replace('{player}', $player->getName(), $award['command']));
				$player->sendMessage(str_replace(['{quest}', '{award}'], [$this->plugin->config['quests'][$id], $award['name'] ?? 'Permission'], $text));
				return true;
			}
